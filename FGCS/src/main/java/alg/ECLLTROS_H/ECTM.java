...
public class ECTM {
     ...
     * @return 调度策略 + T + R + E
     */
    public static Strategy MinTaskTimeExe(double limitE, Processor[] processors, Task[] tasks ) {
        ...
        for (int i = 0; i < tasks.length; i++) {
            ...
            //赋予评估初值，判断循环(得出当前任务的最低调度长度对应的服务器下标index)
            double compareT = T_taskProcessor[i][0]; int index = 0;
            for (int j = 1; j < processors.length; j++) {
                double newT =  T_taskProcessor[i][j];
                if (newT < compareT && newT != 0){
                    compareT = newT;
                    index = j;
                }
            }
            ...
            //刷新index服务器总执行时间
            Time_P[index] += T_taskProcessor[i][index];
    ...
    public static Strategy MinLevelTimeExe(double limitE, Processor[] processors, Task[] tasks , int h) {
        ...
        double limitE_task = limitE / tasks.length;     //等能量法
        int[] task_Processor = new int[tasks.length];   //存储每个任务(下标)的最终分配处理器(值)
        double[] T_taskExeProcessor = new double[tasks.length];
        double[] Re_taskExeProcessor = new double[tasks.length];
        ...
        for (int i = 0; i < tasks.length; i++) {
            ...
            //赋予评估初值，判断循环(得出当前任务完成分配后层级时间上升最小的服务器下标index)
            double compareT = Time_P[0] + T_taskProcessor[i][0]; int index = 0;

            for (int j = 1; j < processors.length; j++) {
                double newT =  Time_P[j] + T_taskProcessor[i][j];
                if (newT < compareT && newT != 0){
                    compareT = newT;
                    index = j;
                }
            ...
            //刷新index服务器总执行时间
            Time_P[index] += T_taskProcessor[i][index];
            T_taskExeProcessor[i] = T_taskProcessor[i][index];

            Re_taskExeProcessor[i] = Re_taskProcessor[i][index];
            Reliability_P[index] *= Re_taskProcessor[i][index];
        }
        ...
        strategy_Algorithm1.setTime_P(Time_P);
        strategy_Algorithm1.setEnergy(limitE);
        strategy_Algorithm1.setTasks(tasks);
        strategy_Algorithm1.setTaskExeProcessor(task_Processor);
        strategy_Algorithm1.setFinalTime(Final_T);
        strategy_Algorithm1.setReality(level_R);
        strategy_Algorithm1.setT_taskExeProcessor(T_taskExeProcessor);
        strategy_Algorithm1.setRe_taskExeProcessor(Re_taskExeProcessor);
